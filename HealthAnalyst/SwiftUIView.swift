...
struct SleepDashboardView: View {
    ...
    struct AreaChartView: View {
        ...
        // Computed property to determine the start of the day for the x-axis
        ...
        // Computed property to determine the end of the day for the x-axis
        private var endOfDay: Date {
            return Calendar.current.date(byAdding: .day, value: 1, to: startOfDay)!
        }

        // Computed property to create axis marks for every 2 hours within the day
        private var next12Hours: [Date] {
            ...
            let start = calendar.date(bySettingHour: startHour, minute: 0, second: 0, of: firstDataPoint.date)!
            var dates: [Date] = []
            for i in 0..<6 {
        ...
        var body: some View {
            Chart {
                ForEach(sleepData, id: \.id) { dataPoint in
                    AreaMark(
                        ...
                        yStart: .value("Min Duration", dataPoint.sleepStage == 1 ? 0 : minDuration(for: dataPoint)),
            ...
            .chartYScale(domain: 0...maxDuration()) // Safely calculate maxDuration
            .padding(.top, 8)
            .chartLegend(position: .bottom)
        }

        // Helper function to format time for the x-axis labels
        ...
        // Helper function to calculate the minimum duration for a given data point
        private func minDuration(for dataPoint: SleepData) -> Double {
            return 0
        }

        // Helper function to calculate the maximum duration for a given data point
        private func maxDuration(for dataPoint: SleepData) -> Double {
            return dataPoint.duration / 60 // Convert seconds to minutes
        }

        // Safely calculate the maximum duration, handling empty sleepData
        private func maxDuration() -> Double {
            guard let maxDataPoint = sleepData.max(by: { $0.duration < $1.duration }) else {
                return 1.0 // Default value if sleepData is empty
            }
            return maxDataPoint.duration / 60 // Convert seconds to minutes
        }
    }
